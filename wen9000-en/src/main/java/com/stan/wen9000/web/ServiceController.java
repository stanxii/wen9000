...
public class ServiceController {
	...
	private static void doDevmodeSet(String message) throws ParseException{
		...
		switch(Integer.parseInt(devmode)){
			...
			case 1:
				key = "global:WEC-3501I-S220";
				optjson.put("desc", "DeviceType WEC-3501I S220 Displayed as "+value);
				break;
			case 2:
				key = "global:WEC9720EK-C22";
				optjson.put("desc", "DeviceType WEC-3501I S220 Displayed as "+value);
			...
			case 3:
				key = "global:WEC9720EK-S220";
				optjson.put("desc", "DeviceType WEC9720EK S220 Displayed as "+value);
				break;
			case 4:
				key = "global:WEC9720EK-SD220";
				optjson.put("desc", "DeviceType WEC9720EK SD220 Displayed as "+value);
				break;
			case 5:
				key = "global:WEC701-C2";
				optjson.put("desc", "DeviceType WEC701 C2 Displayed as "+value);
				break;
			case 6:
				key = "global:WEC701-C4";
				optjson.put("desc", "DeviceType WEC701 C4 Displayed as "+value);
				break;
			case 7:
				key = "global:3702I-C2";
				optjson.put("desc", "DeviceType 3702I C2 Displayed as "+value);
				break;
			case 8:
				key = "global:3702I-C4";
	...
	private static void doUserDel(String message) throws ParseException{
		...
		optjson.put("time", logtimes);
		optjson.put("user", user);
		optjson.put("desc", "User["+username+"]Deleted.");
		sendoptlog(jedis,optjson);
		redisUtil.getJedisPool().returnResource(jedis);
	}
	...
	private static void doOptPreconfig_batch(String message) throws ParseException{
		...
		optjson.put("time", logtimes);
		optjson.put("user", user);
		optjson.put("desc", "Cnu batch pre-account,From MAC:"+longtomac(tmp1)+",To MAC:"+longtomac(tmp2)+"Template:"+jedis.hget("profileid:"+proid+":entity", "profilename"));
		sendoptlog(jedis,optjson);
		JSONArray array = new JSONArray();
		while(tmp1<=tmp2)
		{
	...
	private static void doOptPreconfig_one(String message) throws ParseException{
		...
		if((jedis.exists("mac:"+mac+":deviceid"))||(jedis.exists("preconfig:"+mac+":entity"))){
			...
			optjson.put("time", logtimes);
			optjson.put("user", user);
			optjson.put("desc", "Cnu["+ mac+"]pre-account,Template:"+jedis.hget("profileid:"+proid+":entity", "profilename"));
			sendoptlog(jedis,optjson);
		}
		redisUtil.getJedisPool().returnResource(jedis);
	}
	...
	private static void doOptFtpupdate(String message) throws ParseException{
		...
		for(Iterator cbat=cbats.iterator();cbat.hasNext();){
			...
			optjson.put("time", logtimes);
			optjson.put("user", user);
			optjson.put("desc", "Cbat["+jedis.hget("cbatid:"+cbatid+":entity", "mac")+"]Upgrade.");
	    	sendoptlog(jedis,optjson);
		}
	...
	private static void doOptSendConfig(String message){
		...
		for(Iterator it = list.iterator(); it.hasNext();){
			...
			optjson.put("desc", "Config cnu["+cnumac+"],Template:"+jedis.hget("profileid:"+proid.trim()+":entity", "profilename"));
			sendoptlog(jedis,optjson);
			...
			if(jedis.exists("profileid:"+old_proid+":entity")){
				//如果原模板是配置信息，则删除
				if(jedis.hget("profileid:"+old_proid+":entity","profilename").equalsIgnoreCase("Configinfo")){
...
//        		break;
	...
	private static void doNodeTreeInit() {
    	...
    	eocjson.put((String)"title", (String)"EOC Device");
