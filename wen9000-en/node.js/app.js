...
app.get('/login', function (req, res) {
	...
	res.render('login.jade', { title: 'Wen9000 NMS---Login' });
});

app.get('/register', function (req, res) {
	res.render('register.jade', { title: 'Wen9000 NMS---Register' });
});
//用户不存在
app.get('/101', function (req, res) {
	res.render('101.jade', { title: 'Wen9000 NMS---ERROR' });
});
//密码错误
app.get('/102', function (req, res) {
	res.render('102.jade', { title: 'Wen9000 NMS---ERROR' });
});
		    ...
		    if(result){
		    	jedis.hget('user:'+req.session.user,"password", function(error, result) {
		    	    if(result == req.session.password){
		    	    	res.render('index', { title: 'Wen9000 NMS' });
		    	    }else{
		    	    	res.redirect('/102');
		    	    }
		    	});
...
app.get('/profilemanager', function( request, response ) {
	if ((request.session.user != null)&&((request.session.user != "undefined"))) {
		response.render( 'profilemanager.jade', { title: 'Wen9000 NMS---Template management' } );
	} else {
...
app.get('/opt/accounts', function( request, response ) {
	if ((request.session.user != null)&&((request.session.user != "undefined"))) {
		response.render( 'opt/accounts.jade', { title: 'Wen9000 NMS---Select CNU' } );
	} else {
		response.redirect('/login');
	}    
});
app.get('/opt/selectprofiles', function( request, response ) {
	if ((request.session.user != null)&&((request.session.user != "undefined"))) {
		response.render( 'opt/selectprofiles.jade', { title: 'Wen9000 NMS---Select template' } );
	} else {
		response.redirect('/login');
	}        
});
app.get('/opt/confirm', function( request, response ) {
	response.render( 'opt/confirm.jade', { title: 'Wen9000 NMS---Confirm' } );	        
});
app.get('/opt/config_results', function( request, response ) {
    response.render( 'opt/config_results.jade', { title: 'Wen9000 NMS---Query result' } );
});
app.get('/dis/search', function( request, response ) {
	if ((request.session.user != null)&&((request.session.user != "undefined"))) {
		response.render( 'discovery/discovery.jade', { title: 'Wen9000 NMS---Device discovery' } );
	} else {
		response.redirect('/login');
	}     
});
app.get('/dis/result', function( request, response ) {
    response.render( 'discovery/result.jade', { title: 'Wen9000 NMS---Result' } );
});
app.get('/global_opt', function( request, response ) {
	if ((request.session.user != null)&&((request.session.user != "undefined"))) {
		response.render( 'global_opt.jade', { title: 'Wen9000 NMS---Global management' } );
	} else {
		response.redirect('/login');
	}      
});
app.get('/opt/updatecbat', function( request, response ) {
	if ((request.session.user != null)&&((request.session.user != "undefined"))) {
		response.render( 'opt/updatecbat.jade', { title: 'Wen9000 NMS---Cbat upgrade' } );
	} else {
		response.redirect('/login');
	}     
});
app.get('/version', function( request, response ) {
    response.render( 'version.jade', { title: 'Wen9000 NMS---Version information' } );
});
app.get('/historyalarm', function( request, response ) {
	if ((request.session.user != null)&&((request.session.user != "undefined"))) {
		response.render( 'historyalarm.jade', { title: 'Wen9000 NMS---Alarm/Opt' } );
	} else {
		response.redirect('/login');
	}       
});
...
app.get('/alarmsetting', function( request, response ) {
	if ((request.session.user != null)&&((request.session.user != "undefined"))) {
		response.render( 'alarmsetting.jade', { title: 'Wen9000 NMS---History alarm' } );
	} else {
		response.redirect('/login');
	}       
});
...
app.get('/optlog', function( request, response ) {
	if ((request.session.user != null)&&((request.session.user != "undefined"))) {
		response.render( 'optlog.jade', { title: 'Wen9000 NMS---Opt Manager' } );
...
app.get('/opt/pre_config', function( request, response ) {
	if ((request.session.user != null)&&((request.session.user != "undefined"))) {
		response.render( 'opt/pre_config.jade', { title: 'Wen9000 NMS---Device pre-config' } );
	} else {
		response.redirect('/login');
	}      
});
app.get('/userManager', function( request, response ) {
	if ((request.session.user != null)&&((request.session.user != "undefined"))) {
		response.render( 'userManager.jade', { title: 'Wen9000 NMS---User management' } );
	} else {
		response.redirect('/login');
	}     
});
app.get('/viewmode', function( request, response ) {
    response.render( 'viewmode.jade', { title: 'Wen9000 NMS---View Mode' } );
});
//app.listen(3000);
