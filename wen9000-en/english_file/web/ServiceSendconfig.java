  ...
  public void start() {
	...
	private static void doPreConfig(String message) throws ParseException{
		...
		if(iserror){
			...
			alarmhash.put("cnalarminfo", "Cbat identification:"+jedis.hget("cbatid:"+jedis.get("mac:"+cbatmac+":deviceid")+":entity", "label")+"'s CNU["+cnumac+"]pre-config fail");
			alarmhash.put("enalarminfo", "CNU["+cnumac+"] Under Cbat["+cbatmac+ "] PreConfig!" );
		}else{
			alarmhash.put("alarmlevel", "5");
			alarmhash.put("cnalarminfo", "Cbat identification:"+jedis.hget("cbatid:"+jedis.get("mac:"+cbatmac+":deviceid")+":entity", "label")+"'s CNU["+cnumac+"]pre-config successs");
			alarmhash.put("enalarminfo", "CNU["+cnumac+"] Under Cbat["+cbatmac+ "] PreConfig!" );
		}
