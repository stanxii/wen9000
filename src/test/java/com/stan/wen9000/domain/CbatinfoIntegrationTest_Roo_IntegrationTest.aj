...
privileged aspect CbatinfoIntegrationTest_Roo_IntegrationTest {
    
    declare @type: CbatinfoIntegrationTest: @RunWith(SpringJUnit4ClassRunner.class);
    
    declare @type: CbatinfoIntegrationTest: @ContextConfiguration(locations = "classpath:/META-INF/spring/applicationContext*.xml");
    
    declare @type: CbatinfoIntegrationTest: @Transactional;
    ...
    @Test
    public void CbatinfoIntegrationTest.testFlush() {
        Cbatinfo obj = dod.getRandomCbatinfo();
        Assert.assertNotNull("Data on demand for 'Cbatinfo' failed to initialize correctly", obj);
        Long id = obj.getId();
        Assert.assertNotNull("Data on demand for 'Cbatinfo' failed to provide an identifier", id);
        ...
        Assert.assertTrue("Version for 'Cbatinfo' failed to increment on flush directive", (currentVersion != null && obj.getVersion() > currentVersion) || !modified);
    }
